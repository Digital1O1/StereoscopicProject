...
bool troubleShootRectification = false;
...
cv::Mat leftCameraMatrix, rightCameraMatrix, leftDistnaceCoefficients, rightDistanceCoefficents, leftGray, rightGray, combinedChessboardCheck;
...
static void processRequest(Request *request, int cameraID)
{
    ...
    for (auto bufferPair : buffers)
    {
        ...
        */
        ...
        if (img == nullptr)
        {
            std::cout << "--------- Error: Image buffer is null ---------" << std::endl;
        }
        ...
        if (has_image0 && has_image1 && calibrationInProgress && !calibrationComplete)
        {
            ...
            if (foundLeft && foundRight)
            {
                ...
                {
                    printf("\nSaving chessboard images for sanity check\r\n");
                    imwrite("leftCheck.jpeg", leftGray);
                    imwrite("rightGray.jpeg", rightGray);
                    ...
                    std::cout << "\nrotationMatrix : " << rotationMatrix << "\n\ntranslationVector : " << translationVector << "\n\nessentialMatrix : " << essentialMatrix << "\n\nfundamentalMatrix : " << fundamentalMatrix << std::endl;
                ...
                } // End of if (imagesCaptured == NUMBER_OF_IMAGES)
            } // End of if (foundLeft && foundRight)
            else
            {
                printf("Image not captured. Please reposition calibration chessboard.\r\n");
            }
        } // End of if (has_image0 && has_image1 && calibrationInProgress && !calibrationComplete)

        if (has_image0 && has_image1 && calibrationComplete && troubleShootRectification)
        {
            ...
            cv::Mat concatenatedRectifiedImage = concatenateImages(rectifiedLeft, rectifiedRight);
            displayImage(concatenatedRectifiedImage, "RECTIFIED IMAGE");

            // cv::imshow("rectifiedLeft", rectifiedLeft);
            // cv::imshow("rectifiedRight", rectifiedRight);
        }
        cv::waitKey(1);
