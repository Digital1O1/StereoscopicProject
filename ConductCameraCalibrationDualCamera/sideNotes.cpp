                                ...
                                const cv::Mat &R, const cv::Mat &T)
{
    ...
    for (size_t i = 0; i < objectPoints.size(); ++i)
    {
        ...
        cv::projectPoints(objectPoints[i], R, T, K, distCoeffs, projectedPoints);

        for (size_t j = 0; j < imagePoints[i].size(); ++j)
        {
            cv::Point2f observedPoint = imagePoints[i][j];
            cv::Point2f projectedPoint = projectedPoints[j];
            double error = cv::norm(observedPoint - projectedPoint);
            totalError += error;
            ++totalPoints;
        }
    }
...
int main()
{
    ...
    // Calculate error
    std::vector<cv::Point2f> projectedPoints;
    cv::projectPoints(objectPoints, R, T, K, distCoeffs, projectedPoints);
